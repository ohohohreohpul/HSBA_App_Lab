/* Supplier Scorecard — static build.
   Reads the frozen data.json and renders a glassmorphism analytics dashboard.
   ApexCharts carries the visual storytelling (a health gauge, an annotated
   score distribution and category chart with the review line called out, a
   risk donut, a heatmap and a per-supplier trend). No framework, no backend. */
...
const C = {
  ...
  high: '#dc2626', med: '#d97706', low: '#16a34a',
...
const charts = {};                       // elId -> ApexCharts instance
...
function badge(text, color) {
  return `<span class="badge" style="color:${color};background:${color}1f"><span class="d"></span>${esc(text)}</span>`;
}
function riskBadge(level, prefix = '') { return badge((prefix ? prefix + ' ' : '') + level, RISK_COLOR[level] || C.muted); }
...
function draw(elId, options) {
  if (charts[elId]) { charts[elId].destroy(); }
  const host = document.createElement('div');
  charts[elId] = new ApexCharts(host, options);
  return { host, render: () => charts[elId].render() };
}

/* ---------- KPI story cards ---------- */
function kpiCard(item, i) {
  const dcls = item.dir === 'up' ? ' up' : item.dir === 'down' ? ' down' : '';
  const spark = item.spark ? `<div class="k-spark" id="spark-${i}"></div>` : '';
  const delta = item.delta
    ? `<div class="k-delta${dcls}">${item.pill ? `<span class="pill">${esc(item.pill)}</span>` : ''}${esc(item.delta)}</div>` : '';
  return `<div class="card kpi"><div class="k-label">${esc(item.label)}</div>
    <div class="k-value tnum">${esc(item.value)}</div>${delta}${spark}</div>`;
}
function renderKpis(targetId, items) {
  ...
  items.forEach((it, i) => {
    ...
    const opts = {
      ...baseChart('area', 40),
      series: [{ data: it.spark }],
      stroke: { curve: 'smooth', width: 2 }, colors: [C.accent],
      fill: { type: 'gradient', gradient: { shadeIntensity: 0.4, opacityFrom: 0.35, opacityTo: 0 } },
      grid: { show: false, padding: { top: 0, bottom: 0, left: 0, right: 0 } },
      xaxis: { labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false } },
      yaxis: { show: false }, tooltip: { enabled: false }, chart: { sparkline: { enabled: true }, fontFamily: FONT },
    };
    const c = new ApexCharts($(`#spark-${i}`), opts); c.render();
  });
}
...
function gaugeOptions(pctAbove) {
  return {
    ...baseChart('radialBar', 260),
    series: [Math.round(pctAbove)],
    colors: [C.accent],
    plotOptions: { radialBar: {
      hollow: { size: '62%' }, track: { background: 'rgba(23,28,38,0.06)', strokeWidth: '100%' },
      dataLabels: {
        name: { show: true, offsetY: 22, color: C.muted, fontSize: '13px', fontWeight: 600 },
        value: { show: true, offsetY: -14, color: C.ink, fontSize: '40px', fontFamily: 'Space Grotesk',
          fontWeight: 700, formatter: (v) => v + '%' },
      },
    } },
    fill: { type: 'gradient', gradient: { shade: 'light', type: 'diagonal', gradientToColors: [C.accent2],
      stops: [0, 100] } },
...
function donutOptions(counts) {
  ...
  return {
    ...baseChart('donut', 300),
    series: order.map((k) => counts[k] || 0),
    labels: order, colors: order.map((k) => RISK_COLOR[k]),
    legend: { position: 'bottom', fontSize: '13px', labels: { colors: C.ink2 }, markers: { radius: 4 } },
    stroke: { width: 2, colors: ['#ffffff'] },
    plotOptions: { pie: { donut: { size: '66%', labels: {
      show: true, total: { show: true, label: 'Suppliers', color: C.muted, fontSize: '13px', fontWeight: 600,
        formatter: () => order.reduce((s, k) => s + (counts[k] || 0), 0) },
...
function distOptions(dist, threshold) {
  ...
  const colors = dist.counts.map((_, i) => {
    const center = (dist.edges[i] + dist.edges[i + 1]) / 2;
    return center < threshold ? C.high : C.accent;
  });
  return {
    ...baseChart('bar', 300),
    series: [{ name: 'Suppliers', data: dist.counts }],
    xaxis: { categories: cats, tickAmount: 8, labels: { style: { colors: C.muted, fontSize: '11px' } },
      axisBorder: { color: C.grid }, axisTicks: { color: C.grid },
      title: { text: 'Overall score', style: { color: C.muted, fontWeight: 500 } } },
    yaxis: { labels: { style: { colors: C.muted } } },
    plotOptions: { bar: { columnWidth: '78%', borderRadius: 3, distributed: true } },
    colors, legend: { show: false },
    annotations: { xaxis: [{ x: threshold.toFixed(2), strokeDashArray: 5, borderColor: C.ink,
      label: { text: `review line ${threshold.toFixed(1)}`, orientation: 'horizontal',
        style: { background: C.ink, color: '#fff', fontFamily: FONT, fontSize: '11px' } } }] },
...
function countryOptions(rows) {
  return {
    ...baseChart('bar', 340),
    series: [{ name: 'Suppliers', data: rows.map((r) => r.count) }],
    ...
    colors: [C.accent], dataLabels: { enabled: true, style: { colors: [C.ink2], fontSize: '11px' },
      offsetX: 16, formatter: (v) => v },
  };
}
...
function categoryOptions(rows, threshold) {
  ...
  return {
    ...
    dataLabels: { enabled: true, offsetX: 22, style: { colors: [C.ink2], fontSize: '11px' },
...
function heatOptions(cells) {
  ...
  return {
    ...baseChart('heatmap', 300),
    series,
    colors: [C.accent],
    dataLabels: { enabled: true, style: { colors: ['#fff'], fontSize: '11px' }, formatter: (v) => v.toFixed(1) },
    xaxis: { labels: { style: { colors: C.muted } } },
    yaxis: { labels: { style: { colors: C.ink2, fontSize: '12px' } } },
    plotOptions: { heatmap: { radius: 4, enableShades: false, colorScale: { ranges: [
      { from: 0, to: 2.5, color: C.high, name: 'weak' },
      { from: 2.5, to: 3.5, color: C.med, name: 'fair' },
      { from: 3.5, to: 5, color: C.low, name: 'strong' },
    ] } } },
    tooltip: { y: { formatter: (v) => v.toFixed(2) + ' / 5' } },
    legend: { position: 'bottom', labels: { colors: C.ink2 } },
  };
}
...
function trendOptions(trend, threshold) {
  return {
    ...baseChart('line', 260),
    series: [{ name: 'Avg. overall', data: trend.map((t) => ({ x: t.quarter, y: t.overall })) }],
    stroke: { curve: 'smooth', width: 3 }, colors: [C.accent], markers: { size: 4, colors: [C.accent], strokeColors: '#fff' },
    xaxis: { labels: { style: { colors: C.muted, fontSize: '11px' } } },
    yaxis: { min: 1, max: 5, tickAmount: 4, labels: { style: { colors: C.muted } } },
    annotations: { yaxis: [{ y: threshold, strokeDashArray: 5, borderColor: C.high,
      label: { text: `review line ${threshold.toFixed(1)}`, style: { background: C.high, color: '#fff', fontSize: '11px' } } }] },
    fill: { type: 'gradient', gradient: { opacityFrom: 0.25, opacityTo: 0 } },
    tooltip: { y: { formatter: (v) => v.toFixed(2) + ' / 5' } },
  };
}

function chartCard(elId, title, sub, takeaway) {
  const el = $('#' + elId);
  el.innerHTML = `<h3>${esc(title)}</h3><div class="sub">${esc(sub)}</div>
...
function renderOverview() {
  ...
  const above = t.suppliers - t.below_threshold;
  const pctAbove = above / t.suppliers * 100;

  $('#hero-head').textContent = t.below_threshold
    ? `${t.below_threshold} of ${t.suppliers} suppliers need a review.`
    : `All ${t.suppliers} suppliers are above the line.`;

  const gEl = $('#hero-gauge');
  charts['hero-gauge'] = new ApexCharts(gEl, gaugeOptions(pctAbove));
  charts['hero-gauge'].render();
  ...
  renderKpis('#kpis', [
    ...
    { label: 'Average score', value: t.avg_score.toFixed(2),
      pill: `${overAvg >= 0 ? '+' : ''}${overAvg.toFixed(2)}`, dir: overAvg >= 0 ? 'up' : 'down',
      delta: `vs the ${th.toFixed(1)} review line`, spark: META.score_distribution.counts },
    { label: 'Below the line', value: t.below_threshold,
      pill: `${Math.round(t.below_threshold / t.suppliers * 100)}%`, dir: t.below_threshold ? 'down' : 'up',
      delta: 'of the book' },
    { label: 'High risk', value: t.high_risk,
      dir: t.high_risk ? 'down' : 'up', pill: t.high_risk ? 'watch' : 'clear',
      delta: `worst: ${worst.name} (${sc(worst.overall)})` },
  ]);
...
function renderAnalytics() {
  if (rendered.analytics) return;
  rendered.analytics = true;
  const t = META.totals, th = META.threshold;
  renderKpis('#an-kpis', [
    { label: 'Suppliers', value: t.suppliers },
    { label: 'Average score', value: t.avg_score.toFixed(2) },
    { label: 'Below threshold', value: t.below_threshold },
    { label: 'High risk', value: t.high_risk },
  ...
  host = chartCard('an-dist', 'Score distribution', 'Red bars fall below the review line.',
    `<b>${t.below_threshold}</b> below · <b>${t.suppliers - t.below_threshold}</b> above.`);
  ...
  const catRows = [...META.category_avg].sort((a, b) => (b.avg || 0) - (a.avg || 0));
  charts['an-category'] = new ApexCharts(host, categoryOptions(catRows, th)); charts['an-category'].render();
...
/* ---------- scorecards board ---------- */
const COLS = [
  { key: 'name', label: 'Supplier' }, { key: 'country', label: 'Country' },
  { key: 'category', label: 'Category' }, { key: 'overall', label: 'Overall', num: true },
  { key: 'risk', label: 'Risk' }, { key: 'low_confidence', label: 'Confidence' },
...
function renderBoard() {
  ...
  $('#board thead').innerHTML = '<tr>' + COLS.map((c) => {
    ...
    return `<th data-key="${c.key}" class="${c.num ? 'num' : ''}">${c.label}${arr}</th>`;
  }).join('') + '</tr>';
  $('#board tbody').innerHTML = rows.map((s) => `<tr data-id="${s.id}">
    ...
    <td>${s.low_confidence ? '<span class="chip low">Low</span>' : 'OK'}</td>
  ...
  $('#count-note').textContent = `${rows.length} of ${SUPPLIERS.length} suppliers`
    + (nBelow ? ` · ${nBelow} below the ${th.toFixed(1)} line` : '') + ' · click a row for the full drilldown';
  ...
  $$('#board tbody tr').forEach((tr) => tr.onclick = () => openDrawer(+tr.dataset.id));
}
...
function openDrawer(id) {
  ...
  const orders = s.orders.map((o) => `<tr><td>${esc(o.order)}</td><td>${esc(o.ordered || '—')}</td>
    <td>${esc(o.delivered || '—')}</td><td class="num">${o.days == null ? '—' : o.days + ' d'}</td>
  ...
  $('#drawer').innerHTML = `<button class="btn ghost close" id="drawer-close">Close</button>
    <h2>${esc(s.name)}</h2>
    <div class="meta-line">${riskBadge(s.risk, 'Risk:')} ${s.low_confidence ? badge(`Low confidence (${s.num_ratings})`, C.med) : badge('Confidence OK', C.low)}${special}</div>
    <div class="meta-line">${esc(s.country)} · ${esc(s.category)} · ${esc(s.email || '—')}</div>
    <div class="tiles" style="margin-top:18px">
      ${tile('Overall', sc(s.overall))}${tile('Orders', s.num_orders, `${s.num_delivered} delivered`)}
      ${tile('Total spend', eur(s.total_spend))}${tile('Cancelled', s.num_cancelled, s.cancel_rate == null ? '' : `${pct(s.cancel_rate)} of resolved`)}</div>
    ...
    <div class="grid-2" style="margin-top:24px">
      <div><h3>Threshold violations</h3>${violations.length
        ? violations.map((v) => `<div class="note bad"><b>${esc(v.label)}</b>: ${sc(v.score)} (below ${th.toFixed(1)})</div>`).join('')
        : `<div class="note ok">No component below the ${th.toFixed(1)} line.</div>`}</div>
      <div><h3>Recommendations</h3>${recos.map((r) => `<div class="reco">${esc(r)}</div>`).join('')}</div></div>
    <h3 style="margin:24px 0 6px">Performance trend</h3>
    <div id="drawer-trend"></div>
    <h3 style="margin:24px 0 10px">Orders</h3>
    <div class="tbl-wrap card" style="padding:0"><table><thead><tr>
      <th>Order</th><th>Ordered</th><th>Delivered</th><th class="num">Days</th><th class="num">Amount</th>
      <th>Status</th><th class="num">Qual.</th><th class="num">Comm.</th></tr></thead>
  ...
  if (s.trend && s.trend.length >= 2) {
    charts['trend'] = new ApexCharts(host, trendOptions(s.trend, th));
    setTimeout(() => charts['trend'].render(), 60);      // let the drawer finish opening so width is real
  } else {
    host.innerHTML = '<p class="sub">Not enough dated orders to plot a trend.</p>';
  }
}
...
function initFilters() {
  ...
  $('#search').oninput = (e) => { filters.search = e.target.value; renderBoard(); };
  $('#f-country').onchange = (e) => { filters.country = e.target.value; renderBoard(); };
  $('#f-category').onchange = (e) => { filters.category = e.target.value; renderBoard(); };
  $('#f-risk').onchange = (e) => { filters.risk = e.target.value; renderBoard(); };
  $('#t-below').onchange = (e) => { filters.below = e.target.checked; renderBoard(); };
  $('#t-lowconf').onchange = (e) => { filters.lowconf = e.target.checked; renderBoard(); };
}
async function boot() {
  try {
  ...
  document.body.addEventListener('click', (e) => {
    const n = e.target.closest('[data-view]'); if (n) switchView(n.dataset.view);
  });
